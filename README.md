...
Tutto si configura in Prospect, **Impostazioni account → Landing**:

1. **Attiva la landing pubblica**.
2. **Domini autorizzati**: i domini delle pagine che ospitano il form, uno per riga. Da un dominio non in elenco il form risponde «Questo sito non è autorizzato».
3. **Consenti i test da localhost**: per provare la pagina con un server locale (es. `php -S localhost:8080`). Un file aperto con doppio clic non ha dominio e non passa. In produzione va spento.
4. **Turnstile**: le chiavi si creano su dash.cloudflare.com → Turnstile, con gli stessi domini. La site key va anche in `data-turnstile-sitekey` del form, la secret key solo in Prospect. Per i test in locale Cloudflare fornisce chiavi di prova: site key `1x00000000000000000000AA` con secret `1x0000000000000000000000000000000AA` (passano sempre).
